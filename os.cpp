...
uint64_t alloc_frame(FrameAllocator *fa, Memory *mem) {
    ...
    if (!fa->free_list.empty()) {
        ...
        if (fa->watermark + PAGE_SIZE > fa->limit) return UINT64_MAX; // OOM
        pa = fa->watermark;
...
bool map_page(Process *proc, FrameAllocator *fa, Memory *mem,
    ...
    for (int level = 3; level >= 1; level--) {
        ...
        if (!entry->present) {
            ...
            if (new_pa == UINT64_MAX) return false;
...
uint64_t alloc_and_map(Process *proc, FrameAllocator *fa, Memory *mem,
    ...
    if (pa == UINT64_MAX) return UINT64_MAX;
