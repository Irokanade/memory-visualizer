...
static_assert(NUM_CORES <= 8,   "core_valid is uint8_t — supports at most 8 cores");
static_assert(L2_SETS && (L2_SETS & (L2_SETS - 1)) == 0, "L2_SETS must be power of 2");
...
// Clean eviction from L1: clears this core's bit from the given core_valid field (d or i).
// Used when the L1 line is clean (E or S state) — dirty evictions go through l2_cache_write_way.
...
void l2_cache_fill(L2Set *l2Set, uint8_t core_id, uint64_t tag, uint8_t way, uint8_t *data, MESIState state, uint8_t *core_valid_set, uint8_t *core_valid_clr) {
    // Precondition: caller must have evicted the way first — both core_valid fields must be 0.
    // Hard abort (not assert) so this fires in release builds too.
