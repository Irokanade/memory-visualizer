...
static_assert(L1_SETS && (L1_SETS & (L1_SETS - 1)) == 0,
              "L1_SETS must be power of 2");
static_assert(L1_DTLB_SETS && (L1_DTLB_SETS & (L1_DTLB_SETS - 1)) == 0,
              "L1_DTLB_SETS must be power of 2");
static_assert(L2_DTLB_SETS && (L2_DTLB_SETS & (L2_DTLB_SETS - 1)) == 0,
              "L2_DTLB_SETS must be power of 2");
static_assert(ITLB_SETS && (ITLB_SETS & (ITLB_SETS - 1)) == 0,
              "ITLB_SETS must be power of 2");

bool tlb_lookup(TLBSet *set, uint64_t virtual_page_num,
...
void l1_cache_read_way(L1Set *l1Set, uint8_t way, uint8_t offset, uint8_t *data,
                       uint8_t data_size)
{
    assert(offset + data_size <= LINE_SIZE &&
...
// Only succeeds on M or E (SHARED requires an RFO first — handled by
// cpu_write).
void l1_cache_write_way(L1Set *l1Set, uint8_t way, uint8_t offset,
...
void l1_cache_fill(L1Set *l1Set, uint64_t tag, uint8_t way, uint8_t *line,
                   MESIState state)
{
    l1Set->tag[way] = tag;
